...
const UpdateCommentModal = ({ rating, setMessage, setReload, product }) => {
    const [show, setShow] = useState(false);
    const [comment, setComment] = useState(rating?.content || '');
    const [star, setStar] = useState(rating?.star || 0);
    ...
    const handleSubmit = async e => {
        ...
        try {
            ...
            const rsp = await axios.put(`${process.env.REACT_APP_API_URL}/comment/${rating._id}`, data);
            setMessage({
                type: 'success',
