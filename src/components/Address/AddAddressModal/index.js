...
const AddAddressModal = ({ setMessage, setReload }) => {
    ...
    const handleSubmit = async e => {
        ...
        const isProvince = submitData.province.code !== '';
        const isDistrict = submitData.district.code !== '';
        const isWard = submitData.ward.code !== '';
        ...
        if (!isDistrict || !isFullName || !isPhoneNumber || !isProvince || !isWard || !isDesc) {
            ...
            setIsInvalid({ fullName: false, phoneNumber: false, province: false, district: false, ward: false, description: false });
            try {
                const rsp = await axios.post(`${process.env.REACT_APP_API_URL}/address`, submitData);
                if (rsp.data.success) {
                    setMessage({
                        type: 'success',
    ...
    useEffect(() => {
        setWards([]);
        setSubmitData(prev => { return { ...prev, district: { code: '', name: '' }, ward: { code: '', name: '' } } })
        const fetchApi = async () => {
            try {
    ...
    useEffect(() => {
        setSubmitData(prev => { return { ...prev, ward: { code: '', name: '' } } })
        const fetchApi = async () => {
            try {
